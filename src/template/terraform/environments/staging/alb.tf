resource "aws_lb" "genocs_api_alb" {
  name               = "api-alb"
...
resource "aws_lb_target_group" "genocs_api_tg" {
  health_check {
...
resource "aws_lb_listener" "genocs_listener" {
  load_balancer_arn = aws_lb.genocs_api_alb.arn
  port              = "80"
  protocol          = "HTTP"
